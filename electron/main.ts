...
function ensureTerraformTfvars(): void {
  const cfg = readEnv()
  const projectId = (cfg.PROJECT_ID ?? '').trim()
  if (!projectId) {
    throw new Error('PROJECT_ID is missing. Please complete setup and save configuration first.')
  }

  const region = (cfg.REGION ?? 'us-central1').trim() || 'us-central1'
  const zone = (cfg.ZONE ?? 'us-central1-a').trim() || 'us-central1-a'
  const vmName = (cfg.VM_NAME ?? 'novnc-chrome').trim() || 'novnc-chrome'
  const machineType = (cfg.MACHINE_TYPE ?? 'e2-standard-2').trim() || 'e2-standard-2'
  const diskSizeGb = Number.parseInt((cfg.DISK_SIZE_GB ?? '50').trim(), 10)
  const preemptible = String(cfg.PREEMPTIBLE ?? 'false').toLowerCase() === 'true'
  const subnetCidr = (cfg.SUBNET_CIDR ?? '10.0.0.0/24').trim() || '10.0.0.0/24'

  const tfvars = [
    '# Auto-generated by CloudTab from saved configuration (.env).',
    '# This file is regenerated during deploy/destroy on Windows.',
    '',
    ...
    `disk_size_gb = ${Number.isFinite(diskSizeGb) && diskSizeGb > 0 ? diskSizeGb : 50}`,
...
function runWindowsTerraform(args: string[], options?: { timeoutMs?: number }): Promise<string> {
  ensureTerraformTfvars()
  const terraformDir = join(CORE_DIR, 'terraform')
  const command = args[0] ?? ''
  const effectiveArgs = [...args]

  // Never block on stdin prompts in app-driven runs.
  if (!effectiveArgs.includes('-input=false')) {
    effectiveArgs.push('-input=false')
  }
  // Ensure variable values are loaded from tfvars for non-interactive commands.
  if (['apply', 'destroy', 'plan'].includes(command) && !effectiveArgs.some(a => a.startsWith('-var-file='))) {
    effectiveArgs.push('-var-file=terraform.tfvars')
  }
  ...
  return new Promise((resolve, reject) => {
    const timeoutMs = options?.timeoutMs ?? (command === 'init' ? 3 * 60_000 : 0)
    const proc = spawn('terraform', effectiveArgs, { cwd: terraformDir, env })
    let out = '', err = ''
    let settled = false
    ...
    const timer = timeoutMs > 0
      ? setTimeout(() => {
        ...
        proc.kill()
        finishErr(new Error(
          `terraform ${command} timed out after ${Math.floor(timeoutMs / 60_000)} minutes. ` +
    ...
    proc.on('close', (code) => {
      ...
      else finishErr(new Error(err || `terraform ${command} exited with code ${code}`))
    })
