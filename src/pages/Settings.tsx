...
import { ChevronLeft, AlertCircle, Save, Trash2, Loader2 } from 'lucide-react'
...
export default function Settings() {
  ...
  async function handleDestroy() {
    if (!confirm('This will destroy all GCP infrastructure (VM, VPC, firewall rules). This cannot be undone.\n\nContinue?')) return
    setDestroying(true)
    try { await window.api.tfDestroy() } catch (e: any) { setError(e.message) }
    setDestroying(false)
  }
      ...
      <main className="flex-1 p-5 space-y-6 overflow-y-auto">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Spot VM">
              <select value={config.PREEMPTIBLE ?? 'false'} onChange={set('PREEMPTIBLE')} className="input">
        ...
        <section className="space-y-3">
          ...
          <Field label="Display Resolution">
            <select value={config.RESOLUTION ?? '1920x1080x24'} onChange={set('RESOLUTION')} className="input">
